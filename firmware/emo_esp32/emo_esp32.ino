...
static const float SLEW_DEG_PER_S = 250; // per-joint speed limit (MG90S ~ 600 deg/s unloaded)
...
void saveSettings()
{
    ...
    Settings stored;
    EEPROM.get(0, stored);
    if (memcmp(&stored, &settings, sizeof(Settings)) == 0)
    {
        return; // unchanged: spare the flash a write cycle
    }
...
void controlTick(float dt)
{
    ...
    if (telemetryOn && millis() - lastTelemetryMs >= TELEMETRY_MS)
    {
        ...
        LINK.print(F("T,"));
        LINK.print(lroundf(pitchDeg * 10));
        LINK.print(',');
        LINK.print(lroundf(pitchRate * 10));
        LINK.print(',');
        LINK.print(lroundf(lastCorrection * 10));
        LINK.print(',');
...
void cmdCalibrate()
{
    ...
    saveSettings();
    LINK.print(F("ACK,C,"));
    LINK.println(lroundf(settings.pitchOffset * 100));
}
