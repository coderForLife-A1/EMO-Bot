// EMO-Bot biped firmware (Arduino Nano): IMU pitch balance + walking gait on 4 leg servos.
//
...
void cmdStand()
{
    ...
    Serial.println(imuOk ? F("ACK,S") : F("ACK,S,NOIMU"));
}
...
void setup()
{
    ...
    Wire.setWireTimeout(3000, true); // never hang the control loop on a glitched I2C bus
    ...
    Serial.println(imuOk ? F("READY,IMU") : F("READY,NOIMU"));
    lastTickUs = micros();
}
