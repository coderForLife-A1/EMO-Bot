"""Compile the Nano firmware for the host with stub Arduino headers and run its test scenarios.
...
def harness(tmp_path_factory):
    exe = tmp_path_factory.mktemp("fw") / "fw_harness"
    subprocess.run(["g++", "-std=c++11", "-O1", "-Wall", "-Wextra", "-I", str(HERE), str(HERE / "harness.cpp"),
